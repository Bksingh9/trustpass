...
- Worker deployment: the `Deploy TRUSTPASS Live Worker` workflow can resolve an existing D1 database by name or create it before applying migrations, running the deployed E2E proof, saving the live URL as a repo variable when permitted, and publishing the public gateway preconnected to the Worker URL.
