...
- Render/FastAPI live path: `render.yaml` deploys the production API with managed PostgreSQL, disables demo routes, runs migrations, optionally bootstraps realistic verification seed records, and can be verified with `.github/workflows/verify-deployed-api.yml` via `TRUSTPASS_API_BASE_URL`.
