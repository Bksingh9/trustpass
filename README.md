...
- Worker deployment: the `Deploy TRUSTPASS Live Worker` workflow preflights Cloudflare token/D1 access, resolves an existing D1 database by name or creates it, applies migrations, resolves the deployed Worker URL, runs the deployed E2E proof, saves the live URL as a repo variable when permitted, publishes the public gateway preconnected to that URL, and verifies the public gateway can create and re-read live records through the Worker.
